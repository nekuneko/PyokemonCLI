...
pokemon_legendarios = {'missingno.','moltres','zapdos','articuno','mewtwo','mew','raikou', 
...
max_pokemon = 493 # solo pokemon del 0 al 493, 4ª generación
...
class Pokemon:
	...
	def __init__ (self, int_numero = 0, int_nivel = 5, int_id_entrenador = 0, 
		...
		# Poner movimientos por defecto o de base de datos
		...
		if (str(self.numero) in pokemon_db):
			...
			if (bool_usamosDropbox):
				...
				for i in range (0, 12):
					...
					l_pkm = modulo_dropbox.listarArchivos() 	# vuelvo a consultar la lista
					if (str(self.numero) + ".jpg" in l_pkm): 	# si el pokémon está en dropbox, True
				...
				if (bool_pokemonEnDropbox): 
					...
					#print("El pokemon está en dropbox")
		...
		if (self.nombre.lower() in pokemon_legendarios): # Si Pkm legendario, no tiene sexo
			self.sexo = NOGENERO
	...
	def setMovimientos (self, l4_movimientos = []):
		...
		if (len(self.movimientos) < 4):
			for i in range(len(self.movimientos), 4):
				self.movimientos.append(movimientoToSet(movimientos_db['']))
		...
		# Si el usuario no introdujo movimientos, tirar de base de datos movimientos
